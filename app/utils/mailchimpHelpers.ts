 ...
 */
export async function updateMailchimpAddress(
  email: string,
  ...
  if (!listId) {
    throw new HttpError(
      "MAILCHIMP_LIST_ID is not configured in your environment.",
      400,
    );
  }
...
export async function updateMailchimpTag(
  email: string,
  tagName: string,
  ...
  if (!listId) {
    throw new HttpError(
      "MAILCHIMP_LIST_ID is not configured in my environment.",
      400,
    );
  }
